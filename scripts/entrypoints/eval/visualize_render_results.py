...
for dir_name in dir_names:
    ...
    for i in range(3):
        ...
        for frame_idx in range(frame_len):
            ...
            human_mask_image_path = f"{human_mask_path}/{case_name}/mask/{i}/0/{frame_idx}.png"
            object_image_path = f"{object_mask_path}/{case_name}/mask/{i}/{frame_idx}.png"
            ...
            render_mask = np.logical_and((render_img != 0).any(axis=2), render_img[:, :, 3] > 100)
            ...
            final_image[:, :, :] = render_img[:, :, :3] * test_alpha[:, :, None] + final_image * (
                1 - test_alpha[:, :, None]
            )

            final_image[human_mask] = alpha * origin_img[human_mask] + (1 - alpha) * np.array(
                [255, 255, 255], dtype=np.uint8
            )

...
class State:
    def __init__(self, wp_init_vertices, num_control_points):
        ...
        self.wp_control_x = wp.zeros((num_control_points), dtype=wp.vec3, requires_grad=False)
...
def set_control_points(
    ...
    control_x[tid] = (
        original_control_point[tid] + (target_control_point[tid] - original_control_point[tid]) * t
    )
...
def loop(
    ...
    for k in range(collision_number[i]):
        ...
        if mask1 != mask2 and dis_len < collision_dist and wp.dot(dis, relative_v) < -1e-4:
            valid_count += 1.0

            collision_normal = dis / wp.max(dis_len, 1e-6)
            v_rel_n = wp.dot(relative_v, collision_normal) * collision_normal
            impulse_n = (-(1.0 + clamp_collide_object_elas) * v_rel_n) / (1.0 / m1 + 1.0 / m2)
...
def integrate_ground_collision(
    ...
    if next_x_z < 0.0 and v_z * reverse_factor < -1e-4:
        ...
        a = wp.max(
            0.0,
            1.0 - clamp_collide_fric * (1.0 + clamp_collide_elas) * v_normal_length / v_tao_length,
        )
        v_tao_new = a * v_tao
...
class SpringMassSystemWarp:
    def __init__(
        ...
        if init_velocities is None:
            self.wp_init_velocities = wp.zeros_like(self.wp_init_vertices, requires_grad=False)
        else:
        ...
        self.num_control_points = controller_points.shape[1] if controller_points is not None else 0
        ...
        self.wp_springs = wp.from_torch(init_springs, dtype=wp.vec2i, requires_grad=False)
        ...
        self.wp_collide_object_elas = wp.from_torch(
            torch.tensor([collide_object_elas], dtype=torch.float32, device=self.device),
            requires_grad=cfg.collision_learn,
        )
        self.wp_collide_object_fric = wp.from_torch(
            torch.tensor([collide_object_fric], dtype=torch.float32, device=self.device),
            requires_grad=cfg.collision_learn,
        )
    ...
    def set_controller_interactive(self, last_controller_interactive, controller_interactive):
        # Set the controller points
        wp.launch(
            copy_vec3,
            dim=self.num_control_points,

...
class CameraSystem:
    def __init__(self, WH=None, fps=30, num_cam=3, exposure=50, gain=60, white_balance=3800):
        if WH is None:
            WH = [848, 480]
        self.WH = WH
        self.fps = fps
    ...
    def record(self, output_path):
        ...
        while not self.end:
            if self.recording:
                ...
                timestamps = [
                    last_realsense_data[i]["timestamp"].item() for i in range(self.num_cam)
                ]
                step_idxs = [last_realsense_data[i]["step_idx"].item() for i in range(self.num_cam)]

                if not all([step_idxs[i] == last_step_idxs[i] for i in range(self.num_cam)]):
                    for i in range(self.num_cam):
                        if last_step_idxs[i] != step_idxs[i]:
                            ...
                            cv2.imwrite(f"{output_path}/color/{i}/{step_idx}.png", color)
    ...
    def calibrate(self, visualize=True):
        ...
        while flag:
            ...
            for i in range(self.num_cam):
                ...
                # Calculate the error
                error = np.sqrt(np.sum((reprojected_points - charuco_corners) ** 2, axis=1)).mean()

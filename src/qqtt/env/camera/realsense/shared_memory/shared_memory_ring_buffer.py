...
class SharedMemoryRingBuffer:
    ...
    def __init__(
        ...
        buffer_size = (
            int(np.ceil(put_desired_frequency * get_time_budget * safety_margin)) + get_max_k
        )
    ...
    def put(
        self,
        data: dict[str, np.ndarray | numbers.Number],
        wait: bool = True,
        ...
        if (t - old_timestamp) < self.get_time_budget:
            ...
            if wait:
                ...
                if self.ready_for_get:
                    ...
                    raise TimeoutError(
                        f"[Camera {serial_number}] Put executed too fast {past_iters}items/{deltat:.4f}s ~= {hz}Hz"
                    )

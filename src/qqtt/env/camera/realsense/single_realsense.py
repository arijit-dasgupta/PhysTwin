...
from .shared_memory.shared_memory_queue import Empty, SharedMemoryQueue
            ...
            while not self.stop_event.is_set():
                ...
                for i in range(n_cmd):
                    ...
                    if cmd == Command.SET_COLOR_OPTION.value:
                        ...
                        sensor = (
                            self.pipeline_profile.get_device()
                            .first_depth_sensor()
                            .set_option(rs.option.inter_cam_sync_mode, 1 if self.is_master else 2)
                        )
                        option = rs.option(command["option_enum"])
                        value = float(command["option_value"])
                        sensor.set_option(option, value)

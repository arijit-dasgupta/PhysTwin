...
from qqtt.utils import cfg, logger, visualize_pc
...
# After Phase G, ``gs_render`` lives under ``scripts/shims/``; ensure it is importable without manual PYTHONPATH.
def _ensure_script_shims_on_sys_path() -> None:
    from pathlib import Path

    root = Path(__file__).resolve().parents[3]  # engine/qqtt/src/<repo>
...
_ensure_script_shims_on_sys_path()

import matplotlib.pyplot as plt  # noqa: E402
from gaussian_splatting.dynamic_utils import (  # noqa: E402
    calc_weights_vals_from_indices,
    get_topk_indices,
    interpolate_motions_speedup,
    knn_weights_sparse,
)
from gaussian_splatting.gaussian_renderer import render as render_gaussian  # noqa: E402
from gaussian_splatting.prune_utils import remove_gaussians_with_low_opacity  # noqa: E402
from gaussian_splatting.rotation_utils import (  # noqa: E402
    matrix_to_quaternion,
    quaternion_multiply,
)
from gaussian_splatting.scene.cameras import Camera  # noqa: E402
...
class InvPhyTrainerWarp:
            ...
            if controller_points is not None:
                ...
                for i in range(len(controller_points)):
                    ...
                    for j in idx:
                        ...
                        rest_lengths.append(np.linalg.norm(controller_points[i] - points[j]))
        ...
        for i in range(start_epoch + 1, cfg.iterations):
            ...
            with wp.ScopedTimer("backward"):
                for j in tqdm(range(1, cfg.train_frame)):
                    ...
                    if cfg.data_type == "real":
                        chamfer_loss = wp.to_torch(self.simulator.chamfer_loss, requires_grad=False)
                        track_loss = wp.to_torch(self.simulator.track_loss, requires_grad=False)
    ...
    def test(self, model_path=None):
        if model_path is not None:
            ...
            checkpoint["num_object_springs"]

            assert len(spring_Y) == self.simulator.n_springs, (
    ...
    def visualize_sim(self, save_only=True, video_path=None, save_trajectory=False, save_path=None):
        ...
        vertices = [wp.to_torch(self.simulator.wp_states[0].wp_x, requires_grad=False).cpu()]
    ...
    def init_control_ui(self):
        ...
        self.arrow_empty_orig = cv2.imread("./assets/arrow_empty.png", cv2.IMREAD_UNCHANGED)[
            :, :, [2, 1, 0, 3]
        ]
    ...
    def _overlay_hand_icons(self, frame):
        ...
        if self.n_ctrl_parts == 1:
            ...
            for i in range(2):
                current_target = (
                    self.hand_left_pos.unsqueeze(0) if i == 0 else self.hand_right_pos.unsqueeze(0)
                )
    ...
    def interactive_playground(
        ...
        prev_x = wp.to_torch(self.simulator.wp_states[0].wp_x, requires_grad=False).clone()
        ...
        if n_ctrl_parts > 1:
            hand_positions = []
            for i in range(2):
                target_points = torch.from_numpy(vis_controller_points[self.mask_ctrl_pts[i]]).to(
                    "cuda"
                )
        ...
        if keyboard is not None:
            ...
            logger.warning(
                "pynput not available (headless mode). Keyboard input will be disabled. Use virtual_key_input mode or Gradio interface."
            )
    ...
    def _transform_gs(self, gaussians, M, majority_scale=1):
        ...
        new_scales += torch.log(
            torch.tensor(majority_scale, device=new_scales.device, dtype=new_scales.dtype)
        )
    ...
    def visualize_force(self, model_path, gs_path, n_ctrl_parts=2, force_scale=30000):
        ...
        if n_ctrl_parts == 1:
            ...
            kmeans = KMeans(n_clusters=n_ctrl_parts, random_state=0, n_init=10)
            cluster_labels = kmeans.fit_predict(first_frame_controller_points.cpu().numpy())
            for i in range(n_ctrl_parts):
                force_indexes.append(
        ...
        for i in range(n_ctrl_parts):
            ...
            for j in range(len(control_springs)):
                if (control_springs[j][0] - self.num_all_points) in force_indexes[i]:
                    ...
                    force_rest_lengths[i].append(self.init_rest_lengths[j + num_object_springs])
            ...
            force_rest_lengths[i] = torch.tensor(force_rest_lengths[i], device=cfg.device)
            force_spring_Y[i] = torch.tensor(force_spring_Y[i], device=cfg.device)
            force_object_points[i] = torch.tensor(force_object_points[i], device=cfg.device)
        ...
        if use_white_background:
            image_mask = np.logical_and((image != 1.0).any(axis=2), image[:, :, 3] > 100 / 255)
        else:
            image_mask = np.logical_and((image != 0.0).any(axis=2), image[:, :, 3] > 100 / 255)
        ...
        for j in range(n_ctrl_parts):
            ...
            force_center = torch.mean(prev_x[force_object_points[j]], dim=0).cpu().numpy()
        ...
        for i in tqdm(range(1, frame_len)):
            ...
            for j in range(n_ctrl_parts):
                # Calculate the center of the force_object_points
                force_center = torch.mean(x[force_object_points[j]], dim=0).cpu().numpy()
                # Calculate the force vector
                force_vector = (
                    self.get_force_vector(
                        x,
    ...
    def get_force_vector(
        ...
        with torch.no_grad():
            ...
            spring_forces = (
                torch.clamp(spring_Y, min=cfg.spring_Y_min, max=cfg.spring_Y_max)[:, None]
    ...
    def visualize_material(self, model_path, gs_path, relative_material=True):
        ...
        torch.zeros((self.num_all_points, 3), device=cfg.device)
        torch.zeros(self.num_all_points, dtype=torch.int32, device=cfg.device)
        ...
        normed = (stiffness_map - stiffness_map.min()) / (stiffness_map.max() - stiffness_map.min())
...
def getArrowMesh(origin=None, end=None, color=None):
    if color is None:
        color = [0, 0, 0]
    if origin is None:
        origin = [0, 0, 0]
...
def construct_stiffness_matrix_sparse(
    ...
    k_blocks = coeff[:, None, None] * (d_hat[:, :, None] @ d_hat[:, None, :])  # (N, 3, 3)

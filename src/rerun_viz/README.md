...
**Downsampled spring–mass:** build artifacts with `python -m downsampling.build_downsampled_model ...` (see root `README.md`), then replay with **`--downsample-version <tag>`** (loads `<base_path>/<case_name>/downsampled/<tag>/` and validates the bundle before starting).
